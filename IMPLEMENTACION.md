...
| Campo | Detalle |
|-------|---------|
| **Descripción** | Un Mentor Jedi puede crear cursos grupales, abrirlos al público y cerrarlos. Al abrir un curso, todos los Padawans activos reciben una notificación. |
| **Actores** | 🧙‍♂️ Mentor Jedi |
| **Estado** | ✅ Implementado |
| **Fecha** | 2026-07-12 |
...
| Capa | Archivo | Responsabilidad |
|------|---------|-----------------|
| Frontend | `frontend/src/pages/CoursesPage.tsx` | Grid de cursos, modal de creación, botones Abrir/Cerrar |
| Frontend | `frontend/src/services/api.ts` | `courseService` — endpoints del módulo de cursos |
| Backend | `backend/src/controllers/course.controller.ts` | `createCourse()`, `openCourse()`, `closeCourse()`, `getMyCourses()` |
| Backend | `backend/src/routes/course.routes.ts` | Rutas REST bajo `/api/v1/courses` |
| DB | `backend/src/db/migrations/005_courses.sql` | Tablas `curso` y `curso_inscripcion` |

**Flujo:**
1. Jedi entra a la sección "Cursos" y hace click en "Crear Curso"
2. Llena formulario (título, descripción, categoría, cupo máximo)
3. Backend crea el curso en estado `Borrador` → `POST /api/v1/courses`
4. Jedi hace click en "Abrir curso" → `PATCH /api/v1/courses/:id/open`
5. Backend cambia estado a `Abierto` y notifica a todos los Padawans activos
6. El curso aparece en el catálogo público

---

### ✅ UC-30 — Explorar y unirse a cursos (Padawan)

| Campo | Detalle |
|-------|---------|
| **Descripción** | Un Padawan puede explorar el catálogo de cursos abiertos, ver detalles (título, profesor, cupos) y unirse con un click. También puede abandonar un curso. |
| **Actores** | 🧑‍🎓 Padawan |
| **Estado** | ✅ Implementado |
| **Fecha** | 2026-07-12 |
...
| Capa | Archivo | Responsabilidad |
|------|---------|-----------------|
| Frontend | `frontend/src/pages/CoursesPage.tsx` | Catálogo con tarjetas, botón "Unirme", pestaña "Mis Inscripciones" |
| Frontend | `frontend/src/components/layout/Layout.tsx` | Enlace "Cursos" en el sidebar |
| Backend | `backend/src/controllers/course.controller.ts` | `listCourses()`, `joinCourse()`, `leaveCourse()`, `getCourseDetail()` |
| Backend | `backend/src/routes/course.routes.ts` | `POST /join`, `DELETE /leave` |
| DB | `backend/src/db/migrations/005_courses.sql` | Constraint `UNIQUE(curso_id, padawan_id)`, validación de cupos |

**Flujo:**
1. Padawan entra a "Cursos" → ve grid de cursos abiertos con barra de capacidad
2. Hace click en "Unirme al curso" → `POST /api/v1/courses/:id/join`
3. Backend valida cupo disponible y estado abierto → crea inscripción
4. El Jedi recibe notificación: "[Nombre] se unió a tu curso"
5. El Padawan ve el curso en su pestaña "Mis Inscripciones"
6. Puede abandonar con "Abandonar" → `DELETE /api/v1/courses/:id/leave`

---

### ✅ UC-31 — Sistema de notificaciones mejorado

| Campo | Detalle |
|-------|---------|
| **Descripción** | El sistema dispara notificaciones automáticas en cada evento relevante: sesiones, tareas, calificaciones, inscripciones a cursos, etc. Las notificaciones aparecen en la campana del navbar con badge de no leídas e íconos por tipo. |
| **Actores** | 🧑‍🎓 Padawan · 🧙‍♂️ Mentor Jedi (según el evento) |
| **Estado** | ✅ Implementado |
| **Fecha** | 2026-07-12 |
...
| Capa | Archivo | Responsabilidad |
|------|---------|-----------------|
| Backend | `backend/src/controllers/notification.controller.ts` | Helper `createNotification()` + endpoints CRUD |
| Backend | `backend/src/controllers/session.controller.ts` | Notifica al crear/completar/cancelar sesión |
| Backend | `backend/src/controllers/okr.controller.ts` | Notifica al crear/entregar/calificar tarea |
| Backend | `backend/src/controllers/course.controller.ts` | Notifica al abrir curso y al inscribirse |
| Frontend | `frontend/src/components/layout/Layout.tsx` | Dropdown con íconos por tipo de notificación |
| DB | `backend/src/db/migrations/003_notifications.sql` | 19 tipos de notificación en CHECK constraint |

**Eventos que disparan notificaciones:**

| Evento | Destinatario | Tipo |
|--------|-------------|------|
| Crear sesión | Padawan | `nueva_sesion` |
| Completar sesión | Padawan | `sesion_realizada` |
| Cancelar sesión | Otro participante | `sesion_cancelada` |
| Crear tarea (OKR) | Padawan | `okr_creado` |
| Entregar tarea | Jedi | `okr_entregado` |
| Calificar tarea | Padawan | `okr_completado` |
| Abrir curso | Todos los Padawans | `curso_nuevo` |
| Unirse a curso | Jedi dueño | `curso_inscripcion` |

---

### ✅ UC-32 — Aula Virtual de Cursos (Posts, Comentarios, Estudiantes)

| Campo | Detalle |
|-------|---------|
| **Descripción** | Cada curso tiene un aula virtual donde el Jedi publica anuncios, materiales y tareas. Los Padawans inscritos pueden comentar y participar. La vista incluye un feed de publicaciones y una lista de estudiantes. |
| **Actores** | 🧑‍🎓 Padawan (inscrito) · 🧙‍♂️ Mentor Jedi (propietario del curso) |
| **Estado** | ✅ Implementado |
| **Fecha** | 2026-07-12 |
...
| Capa | Archivo | Responsabilidad |
|------|---------|-----------------|
| DB | `backend/src/db/migrations/006_course_classroom.sql` | Tablas `curso_post` y `curso_comentario` con seed data |
| Backend | `backend/src/controllers/course-classroom.controller.ts` | CRUD de posts, comentarios, pin, feed, estudiantes |
| Backend | `backend/src/routes/course.routes.ts` | Rutas del classroom (`/courses/:id/feed`, `/posts`, etc.) |
| Frontend | `frontend/src/pages/CourseClassroomPage.tsx` | Pagina del aula con banner, feed de posts y tab de estudiantes |
| Frontend | `frontend/src/pages/CoursesPage.tsx` | Tarjetas clickables → navegan a `/courses/:id` |
| Frontend | `frontend/src/services/api.ts` | Metodos `getFeed`, `createPost`, `addComment`, etc. |
| Tests | `backend/tests/unit/schemas/course-classroom.schema.unit.test.ts` | 28 tests unitarios de validacion |

**Flujo:**
1. Jedi crea un curso y lo abre (UC-29)
2. Padawan se inscribe (UC-30)
3. Al hacer clic en la tarjeta del curso → se abre el aula virtual (`/courses/:courseId`)
4. El aula muestra un banner con gradiente, titulo, profesor y categoria
5. Tab "Novedades": feed de posts con tipos (anuncio, material, tarea, discusion), comentarios y fijados
6. Tab "Estudiantes": lista de inscritos con fecha de inscripcion + card del profesor
7. Cualquier miembro puede comentar; el Jedi puede fijar/eliminar posts
8. Al publicar, se notifica a todos los estudiantes inscritos
9. Al comentar, se notifica al autor del post

---

### ✅ UC-33 — Calificaciones y Exportación de Notas

| Campo | Detalle |
|-------|---------|
| **Descripción** | Sistema de calificaciones dentro del aula virtual. El Jedi puede asignar notas a estudiantes por tareas/exámenes. Total privacidad: los estudiantes solo ven sus propias notas. Exportación a CSV/Excel de la matriz de notas. |
| **Actores** | 🧑‍🎓 Padawan · 🧙‍♂️ Mentor Jedi |
| **Estado** | ✅ Implementado |
| **Fecha** | 2026-07-12 |
...
| Capa | Archivo | Responsabilidad |
|------|---------|-----------------|
| DB | `backend/src/db/migrations/006_course_classroom.sql` | Tabla `curso_calificacion` |
| Backend | `backend/src/controllers/course-classroom.controller.ts` | CRUD y export CSV. Lógica estricta de privacidad por rol. |
| Backend | `backend/src/routes/course.routes.ts` | Rutas `/courses/:id/grades` y `/export` |
| Frontend | `frontend/src/pages/CourseClassroomPage.tsx` | Tab de calificaciones, modal para calificar y botón exportar |

**Flujo de Privacidad y Exportación:**
1. Jedi entra al tab "Calificaciones": ve lista de todos los estudiantes, botón "Calificar" y "Exportar". El CSV exportado tiene toda la matriz.
2. Padawan entra al tab "Calificaciones": ve *solo* sus propias notas. El CSV exportado tiene solo sus notas.
3. El backend bloquea intentos de acceso forzado validando `req.user.rol` y `padawan_id`.

---

### ✅ UC-34 — Entregas de Tareas (Subida de Archivos)

| Campo | Detalle |
|-------|---------|
| **Descripción** | Los Padawans pueden subir archivos PDF como respuesta a una publicación de tipo "Tarea". Los Jedis pueden ver la lista de entregas, descargar los PDFs y calificarlos. |
| **Actores** | 🧑‍🎓 Padawan · 🧙‍♂️ Mentor Jedi |
| **Estado** | ✅ Implementado |
| **Fecha** | 2026-07-12 |
...
| Capa | Archivo | Responsabilidad |
|------|---------|-----------------|
| DB | `backend/src/db/migrations/007_course_assignments.sql` | Tabla `curso_tarea_entrega` |
| Backend | `backend/src/middleware/upload.middleware.ts` | Configuración de Multer (límite 5MB, solo PDF) |
| Backend | `backend/src/controllers/course-classroom.controller.ts` | Endpoints `submitAssignment` y `getAssignmentSubmissions` |
| Frontend | `frontend/src/pages/CourseClassroomPage.tsx` | Modales de "Entregar Tarea" y "Ver Entregas" |

**Flujo de Subida y Revisión:**
1. Jedi crea un post tipo `tarea`.
2. Padawan ve el post en Novedades con el botón "Entregar Tarea".
3. Padawan selecciona un PDF y (opcionalmente) escribe un comentario. Se guarda en `backend/uploads/assignments/`.
4. Jedi ve el botón "Ver Entregas", que abre la lista de alumnos que ya entregaron.
5. Desde la lista, el Jedi puede "Descargar PDF" y hacer clic en "Calificar" para ponerle la nota.
...
| Métrica | Valor |
|---------|-------|
| **Total Casos de Uso** | 34 |
| **Implementados** | 34 (UC-01 a UC-34) |
| **En progreso** | 0 |
| **Pendientes** | 0 |
| **Avance** | **100%** ✅ |
...
### Pruebas unitarias (Zod schemas)

| Suite | Archivo | # Tests |
|-------|---------|---------|
| Auth schemas | `tests/unit/schemas/auth.schema.unit.test.ts` | 23 |
| OKR schemas | `tests/unit/schemas/okr.schema.unit.test.ts` | ~25 |
| Profile schemas | `tests/unit/schemas/profile.schema.unit.test.ts` | ~18 |
| Session schemas | `tests/unit/schemas/session.schema.unit.test.ts` | ~20 |
| Vacancy schemas | `tests/unit/schemas/vacancy.schema.unit.test.ts` | ~18 |
| **Course schemas** | `tests/unit/schemas/course.schema.unit.test.ts` | **34** |
| **Course Classroom** | `tests/unit/schemas/course-classroom.schema.unit.test.ts` | **32** |
| Auth middleware | `tests/unit/middleware/auth.middleware.unit.test.ts` | ~20 |

### Pruebas de integración (API + DB)

| Suite | Archivo | # Tests |
|-------|---------|---------|
| Auth API | `tests/auth.test.ts` | 5 |
| Sessions API | `tests/sessions.test.ts` | ~10 |
| OKR API | `tests/okr.test.ts` | ~15 |
| Vacancies API | `tests/vacancies.test.ts` | ~12 |
| Notifications API | `tests/notifications.test.ts` | 5 |
| **Courses API** | `tests/courses.test.ts` | **22** |
| IA / Riesgos | `tests/ia.test.ts` | ~5 |

### Pruebas de humo (smoke)

| Suite | Archivo | # Tests |
|-------|---------|---------|
| **Smoke tests** | `tests/smoke/smoke.test.ts` | **20** |
